...
public enum SunKnownOIDs {
    ...
    public static SunKnownOIDs findMatch(String s) {
        s = s.toUpperCase(Locale.ENGLISH);
        SunKnownOIDs res = name2enum.get(s);
...
//    private static final Debug debug = Debug.getInstance("jca");
    ...
    private static final ConcurrentHashMap<String, SunKnownOIDs> name2enum =
            new ConcurrentHashMap<>();

    static {
        Arrays.asList(SunKnownOIDs.values()).forEach(o -> {
            register(o);
        });
    }

    private static void register(SunKnownOIDs o) {
        SunKnownOIDs ov = name2enum.put(o.oid, o);
        if (ov != null) {
    ...
    private SunKnownOIDs(String oid) {
        this.oid = oid;
        this.stdName = name(); // defaults to enum name
        this.aliases = new String[0];
    }

    private SunKnownOIDs(String oid, String stdName, String ... aliases) {
        this.oid = oid;
